...
class Vixen extends EventEmitter {
  constructor(mongo, root) {
    ...
    this.guildsData = new Discord.Collection();
    this.start();
  }
  ...
  async start() {
    ...
    this.bot.once('ready', async () => {
      ...
      this.bot.guilds.cache.forEach(async (guild) => {
        ...
        const emoji = emojis.get('loading');
        if (!emoji) {
          this.log(`Loading emoji is incorrect or does not exist on guild ${guild.id}. Creating it...`, 'warn');
          const newEmoji = await guild.emojis.create(path.join(this.rootDir, 'assets/loading.gif'), 'vixenload');
